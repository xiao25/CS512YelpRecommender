...
public class RecommendBusinessCategory_3 {
	...
	public static List<String> recommendBusiness(int cluster) {
		...
		List<BusinessRatings> listsBasedSimilarity = Recommendation3_Helper.getAllBusinessLists(cluster);
		// System.out.println(listsBasedSimilarity.size());

		//GET the scores of the business categories already existed in the current geo cluster
		HashMap<String, Double> listsBasedGeo = Recommendation3_Helper.businessScoreInCluster(cluster, "targetCluster");
		HashMap<String, Double> normalizedGeoLists = Recommendation3_Helper.normalize(listsBasedGeo);
