...
public class Recommendation1_Helper {
	...
	public static HashMap<String, Integer> getBusinessCluster() {
		//Key is the business_id, values is its corresponding cluster
		...
		try {
			BufferedReader reader = new BufferedReader(new FileReader(
					"/Users/haowang/Desktop/CS512_Project/business_cluster_hard_mixed.txt"));
	...
	public static HashMap<String, Double> getAllBusinessVisistedByLocalPeople(int cluster) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if(clusterOfReviewedBusiness == -1) continue;
